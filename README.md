...
Созвездия генерируются с применением кода Грея. Соседние точки созвездия отличаются не более чем на 1 бит. Это уменьшает BER на приеме, т.к,  если из-за шума демодулятор ошибется с выбором передаваемого символа, то будет 1 ошибка (если демодулятор перепутал соседние точки).

#### Тесты

Для уверенности в правильности созвездия в файле **tests/QAM_modem.cpp** выполняется генерация и запись в файл всех созвездий. В корне проекта находится Python скрипт, отрисовывающий точки созвездия с подписями в виде соотвуствующих им битов.
...
Сторонние расчеты теоретического BER на MATLAB:
