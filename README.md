...
## Примечание
Обратите внимание на файлы **main.cpp** -> **simulation.cpp** -> **QAM_modem.cpp** -> **channel.cpp**. В этих файлах собраны основные функции, методы и их вызовы.
...
В проекте есть 3 основных класса: **QAM_modulator**, **QAM_demodulator**, **channel**. 

**QAM_modulator** и **QAM_demodulator** имеют общий функционал (они оба должны генерировать, хрнаить созвездия). Чтобы не копировать код, создан класс QAM_base, который содержит в себе общий функционал, а классы **QAM_modulator**, **QAM_demodulator** наследуют этот класс, наследуя этот функционал.

Подробнее на структуру классов можно посмотреть в **/include/QAM_modem.hpp**
...
Дополнительно замеряю время работы модели, считаю MSE между теоретическим и эмпирическим BER (для сравнения времени работы с hard и soft демодуляцией).

![](images/HARD_DEMOD_OUT.png)
![](images/SOFT_DEMOD_OUT.png)


Hard демодуляция справилась быстрее и не потеряла в качестве.